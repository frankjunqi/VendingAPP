...
public class RetreatDetailDbOper {
    public List<VendingChnProductWrapperData> findVendingChnProductWrapperDataByRtId(String rt1Id) {
        List<VendingChnProductWrapperData> list = new ArrayList();
        Cursor c = AssetsDatabaseManager.getManager().getDatabase().rawQuery("SELECT p.PD1_Name,r.Rt2_PlanQty,c.*,s.vs1_quantity FROM RetreatDetail r,Product p,VendingChn c ,vendingchnstock s WHERE c.VC1_CODE=r.RT2_VC1_Code and p.PD1_Id=r.RT2_PD1_Id and r.Rt2_rt1_id=? and c.VC1_CODE=s.VS1_vc1_code and p.PD1_Id=s.vs1_PD1_Id and s.vs1_vd1_id=c.VC1_VD1_ID", new String[]{rt1Id});
        while (c.moveToNext()) {
    ...
    public List<RetreatDetailData> findRetreatDetailDataByRtId(String rt1Id) {
        List<RetreatDetailData> list = new ArrayList();
        Cursor c = AssetsDatabaseManager.getManager().getDatabase().rawQuery("SELECT * from RetreatDetail WHERE Rt2_rt1_id=?", new String[]{rt1Id});
        while (c.moveToNext()) {
            RetreatDetailData data = new RetreatDetailData();
            data.setRt2ActualQty(Integer.valueOf(c.getInt(c.getColumnIndex("RT2_ActualQty"))));
            ...
            data.setRt2ActualQty(Integer.valueOf(c.getInt(c.getColumnIndex("RT2_ActualQty"))));
            data.setRt2PlanQty(Integer.valueOf(c.getInt(c.getColumnIndex("RT2_PlanQty"))));
