...
public class VendingCardPowerDbOper {
    public List<VendingCardPowerData> findAll() {
        ...
        Cursor c = AssetsDatabaseManager.getManager().getDatabase().rawQuery("SELECT * FROM VendingCardPower", null);
        while (c.moveToNext()) {
    ...
    public VendingCardPowerData findCardPower(String vc2Id) {
        Cursor c = AssetsDatabaseManager.getManager().getDatabase().rawQuery("SELECT * FROM VendingCardPower where VC2_ID=?", new String[]{vc2Id});
        VendingCardPowerData vendingCardPower = new VendingCardPowerData();
        if (c.moveToNext()) {
            vendingCardPower.setVc2Id(c.getString(c.getColumnIndex("VC2_ID")));
            vendingCardPower.setVc2M02Id(c.getString(c.getColumnIndex("VC2_M02_ID")));
    ...
    public VendingCardPowerData getVendingCardPower(String cusId, String vendingId, String cardId) {
        Cursor c = AssetsDatabaseManager.getManager().getDatabase().rawQuery("SELECT VC2_ID,VC2_CU1_ID,VC2_VD1_ID,VC2_CD1_ID FROM VendingCardPower WHERE VC2_CU1_ID=? and VC2_VD1_ID=? and VC2_CD1_ID=? limit 1", new String[]{cusId, vendingId, cardId});
        if (!c.moveToNext()) {
            return null;
        }
        VendingCardPowerData vendingCardPower = new VendingCardPowerData();
    ...
    public VendingCardPowerData getVendingCardPower(String vendingId, String cardId) {
        Cursor c = AssetsDatabaseManager.getManager().getDatabase().rawQuery("SELECT VC2_ID,VC2_CU1_ID,VC2_VD1_ID,VC2_CD1_ID FROM VendingCardPower WHERE VC2_VD1_ID=? and VC2_CD1_ID=? limit 1", new String[]{vendingId, cardId});
        if (!c.moveToNext()) {
            return null;
        }
        VendingCardPowerData vendingCardPower = new VendingCardPowerData();
    ...
    public boolean addVendingCardPower(VendingCardPowerData vendingCardPower) {
        SQLiteStatement stat = AssetsDatabaseManager.getManager().getDatabase().compileStatement("insert into VendingCardPower(VC2_ID,VC2_M02_ID,VC2_CU1_ID,VC2_VD1_ID,VC2_CD1_ID,VC2_CreateUser,VC2_CreateTime,VC2_ModifyUser,VC2_ModifyTime,VC2_RowVersion)values(?,?,?,?,?,?,?,?,?,?)");
    ...
    public boolean batchAddVendingCardPower(List<VendingCardPowerData> list) {
        String insertSql = "insert into VendingCardPower(VC2_ID,VC2_M02_ID,VC2_CU1_ID,VC2_VD1_ID,VC2_CD1_ID,VC2_CreateUser,VC2_CreateTime,VC2_ModifyUser,VC2_ModifyTime,VC2_RowVersion)values(?,?,?,?,?,?,?,?,?,?)";
